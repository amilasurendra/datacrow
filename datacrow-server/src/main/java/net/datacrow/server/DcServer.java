...
public class DcServer implements Runnable, IStarterClient, IClient {
	...
	public static void main(String[] args) {
        ...
        if (CoreUtilities.isEmpty(dataDir)) {
            ...
            if (server.initialize(username, password, db)) {
                ...
                if (connector != null) {
                    ...
                    try {
                        connector.setServerAddress(Inet4Address.getLocalHost().getHostAddress());
                    } catch (Exception e) {
                        logger.error("Unable to establish the IP address for this server.");
                    }
                }
